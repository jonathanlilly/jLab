...
%/**********************************************************************
%Heimdall: 1000 = 2x, 2000=4x, 4000=6x, 8000=10x
%Halimede: 1000 =20 x, 2000 = 30x
N=1000;
...
rng(1);  %set seed
tic;z=maternoise(1,N,1,alpha,h);etime1=toc;
tic;z2=maternoise(1,N,1,alpha,h,'fast');etime2=toc;

[psi,lambda]=sleptap(N,8);
[om,spp,snn]=mspec(z,psi,lambda,'adaptive');    
[om,spp2,snn2]=mspec(z2,psi,lambda,'adaptive');    

[f,sppo,snno]=maternspec(1,N,1,alpha,h);
...
axis tight,ylim([10^-5 10^12.5]),xlabel('Frequency (rad)'),text(10^-2,10^-4,'(a)')
...
axis tight,ylim([10^-5 10^12.5]),xlabel('Frequency (rad)'),text(10^-2,10^-4,'(b)')
packfig(1,2,'columns')
...
z=maternoise(1,4096,1,alphag,lambdag);
for i=1:size(z,2)
    z(:,i)=z(:,i)./vstd(z(:,i),1);
    z(:,i)=z(:,i)-vmean(z(bool,i),1);
end
...
hc.Label.String='Log10 of Absolute Value of R_{zz}^{fGn}(\tau,1)'
