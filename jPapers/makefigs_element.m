...
figure
subplot(1,2,1)
plot(2*pi./f,2*s),xlog,ylog,hlines(2)
xlim(2*pi./[max(f) f(2)]),ylim([10^(-2) 100])
title('Fourier Spectrum')
xlabel('Period')
linestyle k 3k 4D k--
h=legend('Signal + Noise','Signal Only','Noise Only','Predicted Noise','AutoUpdate','off');
plot(2*pi./f,2*s(:,1),'k')
%pos=get(h,'Position');
...
%Not sure why I need a factor of 4 in front of a*fs
plot(2*pi./fs,[mean(squared(w),1)' mean(squared(wo),1)' mean(squared(we),1)'  4*a*fs])
...
make bravotrackcensus sizew index ii jj ff kk ww rr lat lon num fdr C rho frho A R Ro sigma wasfilled
