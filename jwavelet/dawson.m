function z = dawson(x,n)
%DAWSON The Dawson function and its derivatives. [With P.J. Acklam]
%
%   Y = DAWSON(X) is Dawson's integral for each element of X, with X real.
%   
...
%   Derivatives
%
%   YN=DAWSON(X,N) optionally returns the Nth derivatives of the Dawson 
...
%   'dawson --f' generates a sample figure.
%
%   Usage: y=dawson(x);
%          yn=dawson(x,n);
%   __________________________________________________________________
...
for k=1:5
    ...
    reporttest(['DAWSON derivatives match numerical differentiation for n=' int2str(k)],err<1e-3)
end
